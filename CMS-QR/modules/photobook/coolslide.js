...
Editted By Wesley De Groot/WesDeGrootProjects For WesDeGroot Site System (WDGSS)		
...
var trans = "slide(right)"; // default transition between photos
...
function loadpics()
{
	...
	for (i = 0; i < pic_array.length; i++)
	{
		...
		temp2 = temp[1]+ '<br>(' + i + '/' + pic_array.length + ')';  // temp2 = description after ##
...
function init_show()
{
	...
	document.getElementById('pictureholder').innerHTML = '<div style="text-align: center;'+
		...
		'<?php echo $lang['photobook']['loading']; ?>'+
...
function setup_functions()
{
	...
	for (i = 0; i < pictures.length; i++) 
	{
		...
		for (f = 0; f < functions.length; f++)
		{
			...
			{	
				pictures[i].title = ftype[0] + '(' + i + '/' + pictures.length + ')';
			}
			} // end if (ftype[1])
		} // end for
...
function changepic()
{
	...
	". Width:"+document.getElementById('picture').width+", "+document.getElementById('picture').height+". URL: "+pictures[cur_pic].photo.src;
	...
	testingbox.innerHTML += ". Width:"+document.getElementById('picture').width+
							", "+document.getElementById('picture').height+". URL: "+pictures[cur_pic].photo.src;
