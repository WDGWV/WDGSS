...
define('NICUPLOAD_PATH', './data/images'); // Set the path (relative or absolute) to
                                      // the directory to save image files
                                      
define('NICUPLOAD_URI', $conf['site']['url'] . 'data/images');   // Set the URL (relative or absolute) to
...
if (!is_writeable(NICUPLOAD_PATH))
	{
		//exit("CHMOD \"" . NICUPLOAD_PATH . "\" To 777!");
	}
...
$id = isset($_POST['APC_UPLOAD_PROGRESS'])?$_POST['APC_UPLOAD_PROGRESS']:'';
if(empty($id)) {
    $id = isset($_GET['id'])?$_GET['id']:null;
}

if($_SERVER['REQUEST_METHOD']=='POST' && isset($_FILES) && isset($_POST['APC_UPLOAD_PROGRESS'])) { // Upload is complete
if(!function_exists('json_encode')) {
    ...
    $status['done'] = 1;
    $status['width'] = '50';//(($size[0]==0)?'50':$size[0]);
...
function bytes_to_readable( $bytes ) {
 ...
 if ( isset ( $_GET['wysiwygl'] ) )
	{
		switch ( $_GET['wysiwygl'] ) 
		  {
			  case 'js':
				$l = file_get_contents("modules/wysiwyg/nicEdit.js");
				$l = preg_replace("#\.\./nicEditorIcons\.gif#",$conf['site']['url']."wysiwyg/img",$l);
			  ...
			  case 'yt':
			  		$l = file_get_contents("modules/wysiwyg/YoutubeVideo.js");
					header("content-type: text/javascript");
					ob_start();
						eval("/**/ ?>" . $l . "<?php /**/");
						$l = ob_get_contents();
			  ...
			  case 'img':
				$l = file_get_contents("modules/wysiwyg/nicEditorIcons.gif");
			  ...
			  case 'loader':
				$l = file_get_contents("modules/wysiwyg/loader.gif");
				header("content-type: image/gif");
				echo $l;
				exit;
			  break;
			  ...
			  case 'video':
				$l = file_get_contents("modules/wysiwyg/yt.png");
...
function wysiwyg ( $textarea )
	{
		global $conf,$lang;
?>
<script src="<?php echo $conf['site']['url']; ?>wysiwyg/js" type="text/javascript"></script>
<script src="<?php echo $conf['site']['url']; ?>wysiwyg/yt" type="text/javascript"></script>
<script type="text/javascript">
	//<![CDATA[
		bkLib.onDomLoaded(function() {
			new nicEditor({fullPanel : true,maxHeight : 300}).panelInstance('<?php echo $textarea; ?>');
		});
	//]]>
</script>
<?php
